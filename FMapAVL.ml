...
module Raw =
 ...
 struct
       ...
       (match l with
        ...
        | Node (ll, lx, ld, lr, _) ->
          (match I.ge_lt_dec (height ll) (height lr) with
           | Coq_left -> create ll lx ld (create lr x d r)
           | Coq_right ->
             (match lr with
              | Leaf -> assert_false l x d r
          ...
          (match r with
           ...
           | Node (rl, rx, rd, rr, _) ->
             (match I.ge_lt_dec (height rr) (height rl) with
              | Coq_left -> create (create l x d rl) rx rd rr
              | Coq_right ->
                (match rl with
                 | Leaf -> assert_false l x d r
                 | Node (rll, rlx, rld, rlr, _) ->
                   create (create l x d rll) rlx rld (create rlr rx rd rr))))
        | Coq_right -> create l x d r))
    ...
    (fun x d ->
      ...
      | Node (rl, rx, rd, rr, rh) ->
        (match I.gt_le_dec lh (I.add rh I._2) with
         | Coq_left -> bal ll lx ld (join lr x d r)
         | Coq_right ->
           (match I.gt_le_dec rh (I.add lh I._2) with
            | Coq_left -> bal (join_aux rl) rx rd rr
            | Coq_right -> create l x d r))
      in join_aux)
  ...
  | Leaf -> acc
  | Node (l, x, d, r, _) ->
    elements_aux (Coq_cons ((Coq_pair (x, d)), (elements_aux acc r))) l

  (** val elements : 'a1 tree -> (key, 'a1) prod list **)

  let elements m =
    elements_aux Coq_nil m
  ...
  | More (x2, d2, r2, e3) ->
    (match X.compare x1 x2 with
     | OrderedType.EQ ->
       (match cmp0 d1 d2 with
        | Coq_true -> cont (cons r2 e3)
        | Coq_false -> Coq_false)
     | _ -> Coq_false)
  ...
  module Proofs =
   struct
    ...
    | End -> Coq_nil
    | More (x, e0, t0, r) ->
      Coq_cons ((Coq_pair (x, e0)), (app (elements t0) (flatten_e r)))
   end
 end
...
module IntMake_ord =
 ...
 struct
  ...
  let compare_pure s1 s2 =
    compare_cont s1 compare_end (R.cons s2 MapS.Raw.End)

  (** val compare : t -> t -> t OrderedType.coq_Compare **)

  let compare s s' =
    let c = compare_pure s s' in
    (match c with
     | Eq -> OrderedType.EQ
     | Lt -> OrderedType.LT
     | Gt -> OrderedType.GT)
