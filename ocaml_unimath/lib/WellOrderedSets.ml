...
let max_common_initial_is_sub x s t =
...
let max_common_initial_is_common_initial x s t =
...
let coq_TOSubset_plus_point_rel x s _ nSz x0 =
...
let isTotalOrder_TOSubset_plus_point x s z nSz =
...
let hasSmallest_WOSubset_plus_point x s z nSz =
...
let upto_eqn x s t x0 sx tx sT =
  invmap
...
let isInterval x s t le lem ini ne =
...
let common_index x s chain i x0 =
...
let common_index2 _ _ chain x y =
...
let common_index3 _ _ chain x y z =
  let x0 = (Obj.magic x).pr1 in
  let j = (Obj.magic x).pr2 in
  let y0 = (Obj.magic y).pr1 in
  let k = (Obj.magic y).pr2 in
...
let chain_union_rel x s chain x0 y =
...
let chain_union_rel_istrans x s chain x0 y z l m =
...
let chain_union_rel_istotal x s chain x0 y =
...
let chain_union_rel_isTotalOrder x s chain =
...
let chain_union_TOSubset x s schain =
  Obj.magic { pr1 =
...
let chain_union_rel_initial x s schain i =
...
let chain_union_rel_hasSmallest x s chain =
  Obj.magic (fun t t' ->
...
let chain_union_WOSubset x s schain =
  Obj.magic { pr1 =
...
let guided_WOSubset_total x g lem =
  Obj.magic (fun x0 ->
    let c = x0.pr1 in
    let gC = x0.pr2 in
    (fun x1 ->
...
let coq_WOlt_istrans x x0 y z h2 h3 =
  Obj.magic (fun h4 ->
...
let coq_WOlt_to_WOlt' x hX x0 y _ =
