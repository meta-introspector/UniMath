...
(** val eqdec_sig :
    'a1 coq_EqDec -> ('a1 -> 'a2 coq_EqDec) -> ('a1 * 'a2) coq_EqDec **)

let eqdec_sig h h0 x =
  let x0 = let pr1,_ = x in pr1 in
  let x1 = let _,pr2 = x in pr2 in
  (fun y ->
  let y0 = let pr1,_ = y in pr1 in
  let y1 = let _,pr2 = y in pr2 in
  (match eq_dec h x0 y0 with
   | Coq_left -> eq_dec (h0 y0) x1 y1
   | Coq_right -> Coq_right))

(** val internal_eq_rew_dep : 'a1 -> 'a2 -> 'a1 -> 'a2 **)

let internal_eq_rew_dep _ f _ =
  f
