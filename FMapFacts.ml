 ...
 struct
  ...
  let fold_rec f i m hempty hstep =
    ...
    let hstep' = fun k e a m' m'' x ->
      hstep (fst (Coq_pair (k, e))) (snd (Coq_pair (k, e))) a m' m'' __ __ __
        x
    in
    let rec f1 l0 hstep'0 m0 =
      match l0 with
      | Coq_nil -> hempty m0 __
      | Coq_cons (y, l1) ->
        let Coq_pair (k, e) = y in
  ...
  let map_induction x x0 m =
    fold_rec (fun _ _ _ -> Coq_tt) Coq_tt m x (fun k e _ m' m'' _ _ _ x1 ->
      x0 m' m'' x1 k e __ __)
  ...
  let map_induction_bis x x0 x1 m =
    fold_rec_bis (fun _ _ _ -> Coq_tt) Coq_tt m (fun m0 m' _ _ x2 ->
  ...
  let filter f m =
    M.fold (fun k e m0 ->
      match f k e with
      | Coq_true -> M.add k e m0
      | Coq_false -> m0) m M.empty
  ...
  let for_all f m =
    M.fold (fun k e b ->
      match f k e with
      | Coq_true -> b
      | Coq_false -> Coq_false) m Coq_true
  ...
  let exists_ f m =
    M.fold (fun k e b ->
      match f k e with
      | Coq_true -> Coq_true
      | Coq_false -> b) m Coq_false

  (** val partition :
      (M.key -> 'a1 -> bool) -> 'a1 M.t -> ('a1 M.t, 'a1 M.t) prod **)

  let partition f m =
    Coq_pair ((filter f m), (filter (fun k e -> negb (f k e)) m))
  ...
  (** val coq_Partition_In :
      'a1 M.t -> 'a1 M.t -> 'a1 M.t -> M.key -> sumbool **)

  let coq_Partition_In _ m1 _ k =
    F.coq_In_dec m1 k

  (** val update_dec : 'a1 M.t -> 'a1 M.t -> M.key -> 'a1 -> sumbool **)
...
module OrdProperties =
 ...
 struct
  ...
  (** val gtb : (M.key, 'a1) prod -> (M.key, 'a1) prod -> bool **)

  let gtb p p' =
    match M.E.compare (fst p) (fst p') with
    | OrderedType.GT -> Coq_true
    | _ -> Coq_false

  (** val leb : (M.key, 'a1) prod -> (M.key, 'a1) prod -> bool **)

  let leb p p' =
    negb (gtb p p')

  (** val elements_lt :
      (M.key, 'a1) prod -> 'a1 M.t -> (M.key, 'a1) prod list **)
  ...
  (** val max_elt_aux : (M.key, 'a1) prod list -> (M.key, 'a1) prod option **)
  ...
  | Coq_cons (p, l0) ->
    (match l0 with
     | Coq_nil -> Some p
     | Coq_cons (_, _) -> max_elt_aux l0)

  (** val max_elt : 'a1 M.t -> (M.key, 'a1) prod option **)

  let max_elt m =
    max_elt_aux (M.elements m)

  (** val min_elt : 'a1 M.t -> (M.key, 'a1) prod option **)
