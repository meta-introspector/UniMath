...
module Ops =
 ...
 struct
  ...
  | Leaf -> acc
  | Node (_, l, x, r) -> elements_aux (Coq_cons (x, (elements_aux acc r))) l

  (** val elements : tree -> X.t list **)

  let elements =
    elements_aux Coq_nil
  ...
  | Leaf -> acc
  | Node (_, l, x, r) ->
    rev_elements_aux (Coq_cons (x, (rev_elements_aux acc l))) r

  (** val rev_elements : tree -> X.t list **)

  let rev_elements =
    rev_elements_aux Coq_nil
  ...
  | Node (_, l, x, r) ->
    (match match f x with
           | Coq_true -> Coq_true
           | Coq_false -> exists_ f l with
     | Coq_true -> Coq_true
     | Coq_false -> exists_ f r)
       ...
       (match l with
        ...
        | Node (_, ll, lx, lr) ->
          (match I.leb (height lr) (height ll) with
           | Coq_true -> create ll lx (create lr x r)
           | Coq_false ->
             (match lr with
              | Leaf -> assert_false l x r
              | Node (_, lrl, lrx, lrr) ->
          ...
          (match r with
           ...
           | Node (_, rl, rx, rr) ->
             (match I.leb (height rl) (height rr) with
              | Coq_true -> create (create l x rl) rx rr
              | Coq_false ->
                (match rl with
                 | Leaf -> assert_false l x r
                 | Node (_, rll, rlx, rlr) ->
                   create (create l x rll) rlx (create rlr rx rr))))
        | Coq_false -> create l x r))
    ...
    (fun x ->
      ...
      | Node (rh, rl, rx, rr) ->
        (match I.ltb (I.add rh I._2) lh with
         | Coq_true -> bal ll lx (join lr x r)
         | Coq_false ->
           (match I.ltb (I.add lh I._2) rh with
            | Coq_true -> bal (join_aux rl) rx rr
            | Coq_false -> create l x r))
      in join_aux)
  ...
  let rec remove_min l x r =
    ...
    | Leaf -> Coq_pair (r, x)
    | Node (_, ll, lx, lr) ->
      let Coq_pair (l', m) = remove_min ll lx lr in Coq_pair ((bal l' x r), m)
  ...
  | Leaf -> { t_left = Leaf; t_in = Coq_false; t_right = Leaf }
  | Node (_, l, y, r) ->
    (match X.compare x y with
     | Eq -> { t_left = l; t_in = Coq_true; t_right = r }
     | Lt ->
    ...
    | Node (_, l1, x1, r1) ->
      (match s2 with
       ...
       | Node (_, _, _, _) ->
         ...
         (match pres with
          | Coq_true -> join (inter l1 l2') x1 (inter r1 r2')
          | Coq_false -> concat (inter l1 l2') (inter r1 r2')))
    ...
    | Node (_, l1, x1, r1) ->
      (match s2 with
       ...
       | Node (_, _, _, _) ->
         let { t_left = l2'; t_in = pres; t_right = r2' } = split x1 s2 in
         (match pres with
          | Coq_true -> concat (diff l1 l2') (diff r1 r2')
          | Coq_false -> join (diff l1 l2') x1 (diff r1 r2')))
  ...
  | Node (_, l, x, r) ->
    ...
    (match f x with
     | Coq_true -> join l' x r'
     | Coq_false -> concat l' r')

  (** val partition : (elt -> bool) -> t -> (t, t) prod **)

  let rec partition f = function
  | Leaf -> Coq_pair (Leaf, Leaf)
  | Node (_, l, x, r) ->
    let Coq_pair (l1, l2) = partition f l in
    let Coq_pair (r1, r2) = partition f r in
    (match f x with
     | Coq_true -> Coq_pair ((join l1 x r1), (concat l2 r2))
     | Coq_false -> Coq_pair ((concat l1 r1), (join l2 x r2)))
 end
...
module MakeRaw =
 ...
 struct
  ...
  | End -> Coq_nil
  | More (x, t0, r) -> Coq_cons (x, (app (elements t0) (flatten_e r)))
