...
module N =
 struct
  ...
  let ggcd a b =
    match a with
    | N0 -> Coq_pair (b, (Coq_pair (N0, (Npos Coq_xH))))
    | Npos p ->
      (match b with
       | N0 -> Coq_pair (a, (Coq_pair ((Npos Coq_xH), N0)))
       | Npos q ->
         let Coq_pair (g, p0) = Pos.ggcd p q in
         let Coq_pair (aa, bb) = p0 in
         Coq_pair ((Npos g), (Coq_pair ((Npos aa), (Npos bb)))))
  ...
  | Npos p ->
    ...
    (match m with
     | Pos.IsPos r -> Coq_pair ((Npos s), (Npos r))
     | _ -> Coq_pair ((Npos s), N0))
