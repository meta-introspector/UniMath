 ...
 struct
  ...
  | Coq_pair (s, y) ->
    (match y with
     | IsPos r ->
       ...
       let r' = g (f r) in
       (match leb s' r' with
        | Coq_true -> Coq_pair ((Coq_xI s), (sub_mask r' s'))
        | Coq_false -> Coq_pair ((Coq_xO s), (IsPos r')))
     | _ ->
       Coq_pair ((Coq_xO s),
         (sub_mask (g (f Coq_xH)) (Coq_xO (Coq_xO Coq_xH)))))
  ...
  | Coq_xI p0 ->
    (match p0 with
     ...
     | Coq_xH -> Coq_pair (Coq_xH, (IsPos (Coq_xO Coq_xH))))
  | Coq_xO p0 ->
    (match p0 with
     | Coq_xI p1 ->
     ...
     | Coq_xH -> Coq_pair (Coq_xH, (IsPos Coq_xH)))
  | Coq_xH -> Coq_pair (Coq_xH, IsNul)
  ...
  let rec ggcdn n a b =
    match n with
    | O -> Coq_pair (Coq_xH, (Coq_pair (a, b)))
    | S n0 ->
      (match a with
       | Coq_xI a' ->
         (match b with
          | Coq_xI b' ->
            (match compare a' b' with
             ...
             | Lt ->
               let Coq_pair (g, p) = ggcdn n0 (sub b' a') a in
               let Coq_pair (ba, aa) = p in
               Coq_pair (g, (Coq_pair (aa, (add aa (Coq_xO ba)))))
             | Gt ->
               let Coq_pair (g, p) = ggcdn n0 (sub a' b') b in
               let Coq_pair (ab, bb) = p in
               Coq_pair (g, (Coq_pair ((add bb (Coq_xO ab)), bb))))
          | Coq_xO b0 ->
            let Coq_pair (g, p) = ggcdn n0 a b0 in
       ...
       | Coq_xO a0 ->
         (match b with
          | Coq_xI _ ->
            let Coq_pair (g, p) = ggcdn n0 a0 b in
            let Coq_pair (aa, bb) = p in
            Coq_pair (g, (Coq_pair ((Coq_xO aa), bb)))
          | Coq_xO b0 ->
            let Coq_pair (g, p) = ggcdn n0 a0 b0 in Coq_pair ((Coq_xO g), p)
          | Coq_xH -> Coq_pair (Coq_xH, (Coq_pair (a, Coq_xH))))
  ...
  (** val of_int : Decimal.signed_int -> positive option **)
  ...
  (** val of_hex_int : Hexadecimal.signed_int -> positive option **)
  ...
  (** val of_num_int : signed_int -> positive option **)
  ...
  (** val to_num_hex_uint : positive -> uint **)

  let to_num_hex_uint n =
    UIntHexadecimal (to_hex_uint n)

  (** val to_int : positive -> Decimal.signed_int **)

  let to_int n =
    Decimal.Pos (to_uint n)

  (** val to_hex_int : positive -> Hexadecimal.signed_int **)

  let to_hex_int p =
    Pos (to_hex_uint p)

  (** val to_num_int : positive -> signed_int **)

  let to_num_int n =
    IntDecimal (to_int n)

  (** val to_num_hex_int : positive -> signed_int **)
